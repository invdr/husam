...
- [x] `/catalog` и `/projects` сохраняют текущую страницу в URL (`page`) и передают query-параметры в детальные страницы, чтобы возврат не сбрасывал список на первую страницу.
