...
> Исторический документ: production уже работает на PocketBase (`https://api.husam.ru`). Используйте этот файл как контекст по миграции, backup/rollback и решениям, но не как текущий чеклист статуса.
